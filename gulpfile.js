...
gulp.task("html",function(){
    ...
    gulp.src("src/*.html")    //找到文件下的所有文件
        .pipe(htmlmin(options))//复制到dist/images中
        .pipe(gulp.dest("dist"))//复制到dist/images中
        .pipe(reload({
            stream:true
        }));//自动重新刷新页面
});
...
gulp.task('serve', function() {
    ...
    gulp.watch(["src/styles/*.less","!src/styles/_*.less"],["style"]);
});
